...
if (areaEle) {
    getData("/api/areas").then((data) => {
        ...
        // أضف خيار "كل المناطق" هنا
        areaEle.innerHTML = `<option value="">كل المناطق</option>`;
    });
}
...
if (govEle) {
    ...
    getData("/api/govs").then((data) => {
        govEle.innerHTML = `<option value="" disabled selected>المحافظة</option>`;
        ...
        setTimeout(() => {
            // إضافة "كل المناطق" في حالة التصفية
            areaEle.innerHTML = `<option value="">كل المناطق</option>`;
    ...
    govEle.addEventListener("change", (e) => {
        localStorage.setItem("select_gov", e.target.value);
        areaEle.innerHTML = `<option value="">كل المناطق</option>`; // دا مهم هنا كمان
