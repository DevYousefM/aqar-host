<section>
    ...
    <form method="post" action="{{ route('update.company') }}" class="mt-6 space-y-6">
        @csrf
        @method('post')

        <div>
            <x-input-label for="company_name" :value="__('أسم الشركة')" />
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'update-brief')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                    class="text-sm text-gray-600">{{ __('تم تغيير نبذة الشركة.') }}</p>
            @endif
        </div>
    </form>
</section>
