...
@section('content')
    <!-- Start Sign In -->
    <div class="back-gr">
        <h1 class="pe-5 pt-5 text-white">تعديل العقار</h1>
    </div>

    <div class="sign-in pt-3 m-auto mt-5 mb-5">
        <div class="container">
            <div class="box">
                <h4 class="border-bottom border-2 border-danger pb-1 d-inline-block"> تعديل عقار :</h4>
                ...
                <form class="d-flex flex-column align-items-center gap-2" method="POST" action="{{ route('property.update',$property->id) }}"
                    ...
                    <input value="{{ $property->title }}" class="w-75 p-1 border" type="text" name="title">
                    @error('title')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror

                    <label class="mt-2" for="brief">نبذه عن العقار</label>
                    <textarea class="w-75 p-1 border" name="brief">{{ $property->brief }}</textarea>
                    @error('brief')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                    ...
                    <select name="type" id="type" class="w-75 p-1 border">
                        <option value='0' {{ $property->type == 0 ? 'selected' : '' }}>اختر نوع العقار</option>
                        <option value='شقق' {{ $property->type == 'شقق' ? 'selected' : '' }}>شقق</option>
                    ...
                    <select name="purpose" id="purpose" class="w-75 p-1 border">
                        ...
                        <option value='شراء' {{ $property->purpose == 'شراء' ? 'selected' : '' }}>شراء</option>
                    ...
                    <input value="{{ $property->meters }}" class="w-75 p-1 border" type="number" name="meters">
                    @error('meters')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                    ...
                    <select name="payment" id="payment" class="w-75 p-1 border">
                        ...
                        <option value="كاش" {{ $property->payment === 'كاش' ? 'selected' : '' }}>
                            كاش
                        </option>
                        <option value="قسط" {{ $property->payment === 'قسط' ? 'selected' : '' }}>
                            قسط
                        </option>
                    </select>
                    @error('payment')
                    ...
                    <div id="price" class="d-none flex-column align-items-center gap-2 w-100">
                        ...
                        <input value="{{ $property->price }}" class="w-75 p-1 border" type="number" name="price">
                    </div>
                    @error('price')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                    ...
                    <input value="{{ $property->images }}" id="propertyImages" class="w-75 p-1 border" type="file"
                    ...
                    @if (auth()->user()->has_special)
                        <div class="form-checkbox ">
                            <input class="p-1 m-1 border" {{ $property->is_special ? 'checked' : '' }} type="checkbox"
                                name="is_special">
                        ...
                        @enderror
                    @endif
                    <button class="btn sec-main-btn mt-3 mb-3 fw-bold" type="submit">تعديل العقار</button>
                </form>
            </div>
        </div>
    </div>
